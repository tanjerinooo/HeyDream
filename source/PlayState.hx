...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'blockparty':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/blockparty/blockpartyDialogue'));
		}
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'minecraftday':
					{
						curStage = 'minecraftday';

						var minecraftDay:FlxSprite = new FlxSprite(-245,-150).loadGraphic(Paths.image('minecraft/minecraftday'), false);
						if(FlxG.save.data.antialiasing)
							{
								minecraftDay.antialiasing = true;
							}
						add(minecraftDay);
					}
				case 'philly':
